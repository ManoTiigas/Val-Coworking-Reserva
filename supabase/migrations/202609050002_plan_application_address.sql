...
alter table public.plan_applications
  drop constraint if exists plan_applications_customer_address_required;

alter table public.plan_applications
  add constraint plan_applications_customer_address_required
  check (customer_address is not null and char_length(trim(customer_address)) >= 8) not valid;

drop function if exists public.create_plan_application(text, text, text, text, text, text);
drop function if exists public.create_plan_application(text, text, text, text, text, text, text);

create function public.create_plan_application(
  p_plan_key text,
