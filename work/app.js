...
async function loadSpaces() {
  ...
  $('spaces').innerHTML = data.map((space) => `<button class="choice" data-space="${space.id}"><b>${space.name}</b>${space.code === 'rooftop' ? '<small>Espaço para confraternização</small>' : '<small>Cozinha compartilhada inclusa</small>'}</button>`).join('');
